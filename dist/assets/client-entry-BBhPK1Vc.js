...
function createPageChangeListener(callback) {
  ...
  function stop2() {
    const nav = window.navigation;
    nav == null ? void 0 : nav.removeEventListener("navigate", onNavigate);
    isListening = false;
    lastKey = null;
  }
...
async function waitForContent(iframe, timeout = 15e3) {
  var _a;
  const start2 = Date.now();
  while (Date.now() - start2 < timeout) {
    for (const sel of CONTENT_SELECTORS) {
      const el2 = (_a = iframe.contentDocument) == null ? void 0 : _a.querySelector(sel);
      if (el2 && el2.innerHTML.trim() !== "") return el2;
    }
    await new Promise((r2) => setTimeout(r2, 300));
  }
  throw new Error("コンテンツの読み込みがタイムアウトしました");
}
async function extractRevisionHtml(pageId, revisionId) {
...
var htmldiff_min = function(e) {
  var t2 = {};
  function n2(s) {
    if (t2[s]) return t2[s].exports;
    var r2 = t2[s] = { i: s, l: false, exports: {} };
    return e[s].call(r2.exports, r2, r2.exports, n2), r2.l = true, r2.exports;
  }
  return n2.m = e, n2.c = t2, n2.d = function(e2, t3, s) {
    n2.o(e2, t3) || Object.defineProperty(e2, t3, { configurable: false, enumerable: true, get: s });
  }, n2.r = function(e2) {
  ...
  class r2 {
    constructor(e2, t3, n3) {
      this.startInOld = e2, this.startInNew = t3, this.size = n3;
    }
    get endInOld() {
  ...
  const o = /^\s*<\/?[^>]+>\s*$/, l2 = /<[^\s>]+/, h = /^(\s|&nbsp;)+$/, c = /[\w\#@]+/, d = ["<img"];
  function a(e2) {
    return !d.some((t3) => null !== e2 && e2.startsWith(t3)) && o.test(e2);
  }
  function u2(e2, t3, n3) {
    return ["<", t3, ' class="', n3, '">', e2, "</", t3, ">"].join("");
  }
  function p2(e2) {
    return "<" === e2;
  }
  ...
  function I2(e2) {
    return a(e2) ? function(e3) {
      return e3 = l2.exec(e3)[0] + (e3.endsWith("/>") ? "/>" : ">");
    }(e2) : e2;
  }
  function W2(e2) {
    return c.test(e2);
  ...
  function O2(e2, t3, n3) {
    return e2.push(t3), e2.length > n3 && e2.shift(), e2.length !== n3 ? null : e2.join("");
  }
  class x2 {
    constructor(e2, t3, n3, s2, r3, i2, o2) {
      this.oldWords = e2, this.newWords = t3, this.startInOld = n3, this.endInOld = s2, this.startInNew = r3, this.endInNew = i2, this.options = o2;
    }
    indexNewWords() {
      this.wordIndices = /* @__PURE__ */ new Map();
      let e2 = [];
      for (let t3 = this.startInNew; t3 < this.endInNew; t3++) {
        let n3 = O2(e2, this.normalizeForIndex(this.newWords[t3]), this.options.blockSize);
        null !== n3 && (this.wordIndices.has(n3) ? this.wordIndices.get(n3).push(t3) : this.wordIndices.set(n3, [t3]));
      }
    }
    normalizeForIndex(e2) {
      return e2 = I2(e2), this.options.IgnoreWhiteSpaceDifferences && g(e2) ? " " : e2;
    }
    findMatch() {
      if (this.indexNewWords(), this.removeRepeatingWords(), 0 === this.wordIndices.length) return null;
      let e2 = this.startInOld, t3 = this.startInNew, n3 = 0, s2 = /* @__PURE__ */ new Map();
      const i2 = this.options.blockSize;
      let o2 = [];
      for (let r3 = this.startInOld; r3 < this.endInOld; r3++) {
        let l3 = O2(o2, this.normalizeForIndex(this.oldWords[r3]), i2);
        if (null === l3) continue;
        let h2 = /* @__PURE__ */ new Map();
        if (this.wordIndices.has(l3)) {
          for (let o3 of this.wordIndices.get(l3)) {
            let l4 = (s2.has(o3 - 1) ? s2.get(o3 - 1) : 0) + 1;
            h2.set(o3, l4), l4 > n3 && (e2 = r3 - l4 - i2 + 2, t3 = o3 - l4 - i2 + 2, n3 = l4);
          }
          s2 = h2;
        } else s2 = h2;
      }
      return 0 !== n3 ? new r2(e2, t3, n3 + i2 - 1) : null;
    }
    removeRepeatingWords() {
      let e2 = this.newWords.length + this.options.repeatingWordsAccuracy, t3 = Array.from(this.wordIndices.entries()).filter((t4) => t4[1].length > e2).map((e3) => e3[0]);
      for (let e3 of t3) this.wordIndices.delete(e3);
    }
  }
  class m2 {
    constructor(e2, t3, n3, s2, r3) {
      this.action = e2, this.startInOld = t3, this.endInOld = n3, this.startInNew = s2, this.endInNew = r3;
    }
  }
  var b = { character: 0, tag: 1, whitespace: 2, entity: 3 };
  function N2(e2, t3) {
    let n3 = { mode: b.character, currentWord: [], words: [] }, s2 = function(e3, t4) {
      let n4 = /* @__PURE__ */ new Map();
      if (null === t4) return n4;
      for (let s3 of t4) {
        let t5;
        for (; null !== (t5 = s3.exec(e3)); ) {
          if (n4.has(t5.index)) throw new Error("One or more block expressions result in a text sequence that overlaps. Current expression: " + s3.toString());
          n4.set(t5.index, t5.index + t5[0].length);
        }
      }
      return n4;
    }(e2, t3), r3 = !!s2.size, i2 = false, o2 = -1;
    for (let t4 = 0; t4 < e2.length; t4++) {
      var l3 = e2[t4];
      if (r3) {
        o2 === index && (o2 = -1, i2 = false);
        if (s2.has(index) && (i2 = true, o2 = s2.get(index)), i2) {
          n3.currentWord.push(l3), n3.mode = b.character;
          continue;
        }
      }
      switch (n3.mode) {
        case b.character:
          p2(l3) ? k2(n3, "<", b.tag) : f2(l3) ? k2(n3, l3, b.entity) : g(l3) ? k2(n3, l3, b.whitespace) : W2(l3) && (0 === n3.currentWord.length || W2(n3.currentWord[n3.currentWord.length - 1])) ? n3.currentWord.push(l3) : k2(n3, l3, b.character);
          break;
        case b.tag:
          ">" === l3 ? (n3.currentWord.push(l3), n3.words.push(n3.currentWord.join("")), n3.currentWord = [], n3.mode = g(l3) ? b.whitespace : b.character) : n3.currentWord.push(l3);
          break;
        case b.whitespace:
          p2(l3) ? k2(n3, l3, b.tag) : f2(l3) ? k2(n3, l3, b.entity) : g(l3) ? n3.currentWord.push(l3) : k2(n3, l3, b.character);
          break;
        case b.entity:
          if (p2(l3)) k2(n3, l3, b.tag);
          else if (g(l3)) k2(n3, l3, b.whitespace);
          else if (w2(l3)) {
            let e3 = true;
            if (0 !== n3.currentWord.length && (n3.currentWord.push(l3), n3.words.push(n3.currentWord.join("")), n3.words.length > 2 && g(n3.words[n3.words.length - 2]) && g(n3.words[n3.words.length - 1]))) {
              let t5 = n3.words[n3.words.length - 2], s3 = n3.words[n3.words.length - 1];
              n3.words.splice(n3.words.length - 2, 2), n3.currentWord = [(t5 + s3).split()], n3.mode = b.whitespace, e3 = false;
            }
            e3 && (n3.currentWord = [], n3.mode = b.character);
          } else W2(l3) ? n3.currentWord.push(l3) : k2(n3, l3, b.character);
      }
    }
    return 0 !== n3.currentWord.length && n3.words.push(n3.currentWord.join("")), n3.words;
  }
  function k2(e2, t3, n3) {
    0 !== e2.currentWord.length && e2.words.push(e2.currentWord.join("")), e2.currentWord = [t3], e2.mode = n3;
  }
  const y2 = 4, M2 = /* @__PURE__ */ new Map([["</strong>", 0], ["</em>", 0], ["</b>", 0], ["</i>", 0], ["</big>", 0], ["</small>", 0], ["</u>", 0], ["</sub>", 0], ["</strike>", 0], ["</s>", 0]]), T2 = /<((strong)|(b)|(i)|(em)|(big)|(small)|(u)|(sub)|(sup)|(strike)|(s))[\>\s]+/gi;
  class v2 {
    constructor(e2, t3) {
      this.content = [], this.newText = t3, this.oldText = e2, this.specialTagDiffStack = [], this.newWords = [], this.oldWords = [], this.matchGranularity = 0, this.blockExpressions = [], this.repeatingWordsAccuracy = 1, this.ignoreWhiteSpaceDifferences = false, this.orphanMatchThreshold = 0;
    }
    build() {
      if (this.oldText === this.newText) return this.newText;
      this.splitInputsIntoWords(), this.matchGranularity = Math.min(y2, this.oldWords.length, this.newWords.length);
      let e2 = this.operations();
      for (let t3 of e2) this.performOperation(t3);
    ...
    splitInputsIntoWords() {
      this.oldWords = N2(this.oldText, this.blockExpressions), this.oldText = null, this.newWords = N2(this.newText, this.blockExpressions), this.newText = null;
    }
    performOperation(e2) {
      switch (e2.action) {
        case s.equal:
    ...
    processInsertOperation(e2, t3) {
      let n3 = this.newWords.filter((t4, n4) => n4 >= e2.startInNew && n4 < e2.endInNew);
      this.insertTag("ins", t3, n3);
    }
    processDeleteOperation(e2, t3) {
      let n3 = this.oldWords.filter((t4, n4) => n4 >= e2.startInOld && n4 < e2.endInOld);
      this.insertTag("del", t3, n3);
    }
    processEqualOperation(e2) {
      let t3 = this.newWords.filter((t4, n3) => n3 >= e2.startInNew && n3 < e2.endInNew);
      this.content.push(t3.join(""));
    }
    insertTag(e2, t3, n3) {
      for (; n3.length; ) {
        let s2 = this.extractConsecutiveWords(n3, (e3) => !a(e3)), r3 = "", i2 = false;
        if (0 !== s2.length) {
          let n4 = u2(s2.join(""), e2, t3);
          this.content.push(n4);
        } else {
          if (T2.test(n3[0])) {
            if (this.specialTagDiffStack.push(n3[0]), r3 = '<ins class="mod">', "del" === e2) for (n3.shift(); n3.length > 0 && T2.test(n3[0]); ) n3.shift();
          } else if (M2.has(n3[0])) {
            let t4 = 0 === this.specialTagDiffStack.length ? null : this.specialTagDiffStack.pop();
            if (null !== t4 && t4 === n3[0].replace(/\//g, "") && (r3 = "</ins>", i2 = true), "del" === e2) for (n3.shift(); n3.length > 0 && M2.has(n3[0]); ) n3.shift();
          }
          if (0 === n3.length && 0 === r3.length) break;
          i2 ? this.content.push(r3 + this.extractConsecutiveWords(n3, a).join("")) : this.content.push(this.extractConsecutiveWords(n3, a).join("") + r3);
        }
      }
    }
    extractConsecutiveWords(e2, t3) {
      let n3 = null;
      for (let s2 = 0; s2 < e2.length; s2++) {
        let r3 = e2[s2];
        if (0 === s2 && " " === r3 && (e2[s2] = "&nbsp;"), !t3(r3)) {
          n3 = s2;
          break;
        }
      }
      if (null !== n3) {
        let t4 = e2.filter((e3, t5) => t5 >= 0 && t5 < n3);
        return n3 > 0 && e2.splice(0, n3), t4;
      }
      {
        let t4 = e2.filter((t5, n4) => n4 >= 0 && n4 < e2.length);
        return e2.splice(0, e2.length), t4;
      }
    }
    operations() {
      let e2 = 0, t3 = 0, n3 = [], i2 = this.matchingBlocks();
      i2.push(new r2(this.oldWords.length, this.newWords.length, 0));
      let o2 = this.removeOrphans(i2);
      for (let r3 of o2) {
        let i3, o3 = e2 === r3.startInOld, l3 = t3 === r3.startInNew;
        (i3 = o3 || l3 ? o3 && !l3 ? s.insert : o3 ? s.none : s.delete : s.replace) !== s.none && n3.push(new m2(i3, e2, r3.startInOld, t3, r3.startInNew)), 0 !== r3.length && n3.push(new m2(s.equal, r3.startInOld, r3.endInOld, r3.startInNew, r3.endInNew)), e2 = r3.endInOld, t3 = r3.endInNew;
      }
      return n3;
    }
    *removeOrphans(e2) {
      ...
      for (let s2 of e2) {
        if (null === n3) {
          t3 = new r2(0, 0, 0), n3 = s2;
          continue;
        }
        if (t3.endInOld === n3.startInOld && t3.endInNew === n3.startInNew || n3.endInOld === s2.startInOld && n3.endInNew === s2.startInNew) {
          yield n3;
          t3 = n3;
          n3 = s2;
          continue;
        }
        let e3 = (e4, t4) => e4 + t4.length, i2 = this.oldWords.slice(t3.endInOld, s2.startInOld).reduce(e3, 0), o2 = this.newWords.slice(t3.endInNew, s2.startInNew).reduce(e3, 0);
        this.newWords.slice(n3.startInNew, n3.endInNew).reduce(e3, 0) > Math.max(i2, o2) * this.orphanMatchThreshold && (yield n3), t3 = n3, n3 = s2;
      }
      yield n3;
    }
    matchingBlocks() {
    ...
    findMatchingBlocks(e2, t3, n3, s2, r3) {
      let i2 = this.findMatch(e2, t3, n3, s2);
      null !== i2 && (e2 < i2.startInOld && n3 < i2.startInNew && this.findMatchingBlocks(e2, i2.startInOld, n3, i2.startInNew, r3), r3.push(i2), i2.endInOld < t3 && i2.endInNew < s2 && this.findMatchingBlocks(i2.endInOld, t3, i2.endInNew, s2, r3));
    }
    findMatch(e2, t3, n3, s2) {
      for (let r3 = this.matchGranularity; r3 > 0; r3--) {
        let o2 = new i();
        o2.blockSize = r3, o2.repeatingWordsAccuracy = this.repeatingWordsAccuracy, o2.ignoreWhitespaceDifferences = this.ignoreWhiteSpaceDifferences;
        let l3 = new x2(this.oldWords, this.newWords, e2, t3, n3, s2, o2).findMatch();
        if (null !== l3) return l3;
      }
      return null;
    }
  }
  v2.execute = function(e2, t3) {
    return new v2(e2, t3).build();
  };
  t2.default = v2;
}, function(e, t2, n2) {
  e.exports = n2(0);
}]);
const htmldiff = /* @__PURE__ */ getDefaultExportFromCjs(htmldiff_min);
...
const HtmlDiffView = ({ leftRevision, rightRevision, pageId }) => {
...
const DiffViewer = ({ leftRevision, rightRevision, pageId, mode }) => {
  if (mode === "markdown") {
    return /* @__PURE__ */ jsxRuntimeExports.jsx(MarkdownDiffView, { leftBody: leftRevision.body, rightBody: rightRevision.body });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx(HtmlDiffView, { leftRevision, rightRevision, pageId });
};
const RevisionDiffModal = ({ pageId, isOpen, onClose }) => {
  ...
  const leftRevision = revisions.find((r2) => r2._id === leftId) ?? null;
  const rightRevision = revisions.find((r2) => r2._id === rightId) ?? null;
...
function getSidebarContainer() {
  ...
  return ((_a = primary ?? fallback) == null ? void 0 : _a.parentElement) ?? null;
}
function getButtonClass() {
  const existingBtn = document.querySelector('[data-testid="pageListButton"] button');
  console.log("[revision-diff DEBUG] getButtonClass: existingBtn=", existingBtn, "className=", existingBtn == null ? void 0 : existingBtn.className);
  return (existingBtn == null ? void 0 : existingBtn.className) ?? "btn btn-outline-neutral-secondary rounded-pill py-1 px-lg-3";
}
function mountSidebar(pageId) {
  ...
  if (document.getElementById(MOUNT_ID) != null) {
    ...
    updatePageId == null ? void 0 : updatePageId(pageId);
    return;
  }
...
//# sourceMappingURL=client-entry-BBhPK1Vc.js.map
